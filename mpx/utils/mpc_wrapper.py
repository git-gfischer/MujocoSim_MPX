...
def _whole_body_reference_partial(config, robot_mass, *, clearence_speed_preset: float | None):
    """
    Bind static reference-generator args for whole-body MPC.

    If ``clearence_speed_preset`` is set, bake it into the partial (batch path); otherwise the
    wrapper supplies ``clearence_speed=...`` at each call (live MPC path).
    """
    bind_kw: dict = {
    ...
    if getattr(config, "use_balance_fixed_contact", False):
        bind_kw["fixed_contact_mask"] = getattr(
            config,
            "balance_fixed_contact_mask",
            jnp.ones(config.n_contact, dtype=jnp.float32),
        )
        return partial(mpc_utils.reference_generator_balance, **bind_kw)
    return partial(mpc_utils.reference_generator_locomotion, **bind_kw)
...
class mpx_data(PyTreeNode):
    ...
    grf: jnp.ndarray # Ground reaction force for each foot
...
class BatchedMPCControllerWrapper:
    def __init__(self, config, n_env,limited_memory=False):
        ...
        self.reference_generator = _whole_body_reference_partial(
            config, robot_mass, clearence_speed_preset=0.2
        )
    ...
    def run(self,data, x0, input):
        ...
        reference, parameter, liftoff = self.reference_generator(
            ...
            x = x0,
            foot = x0[13+2*self.config.n_joints:13+2*self.config.n_joints + 3 * n_c],
            input = input,
            liftoff = data.liftoff,
            contact = contact,
            clearence_speed=0.2,
            foot_ref_anchor=jnp.zeros(3 * n_c, dtype=jnp.float32),
            use_foot_ref_anchor=False,
            base_quat_ref=jnp.asarray(x0[3:7], dtype=jnp.float32),
...
class MPCControllerWrapper:
    def __init__(self, config,limited_memory=False):
        ...
        reference_generator = _whole_body_reference_partial(
        ...
        self.use_balance_base_quat_ref = False
        self.balance_base_quat_ref = jnp.asarray(config.quat0, dtype=jnp.float32)
        self._balance_foot_ref_key = jax.random.PRNGKey(0)
        self.use_balance_foot_ref_anchor = True
        self.balance_foot_ref_anchor = jnp.zeros(3 * config.n_contact, dtype=jnp.float32)
        self.last_foot_ref = np.zeros(3 * config.n_contact, dtype=np.float64)
        self.swing_foot_goal_leg = -1
        self.swing_foot_goal_xyz = np.zeros(3, dtype=np.float64)
    ...
    def run(self, qpos, qvel, input, contact):
        ...
        reference, parameter, self.liftoff = self._ref_gen(
            ...
            foot_ref_anchor=self.balance_foot_ref_anchor,
            use_foot_ref_anchor=self.use_balance_foot_ref_anchor,
            base_quat_ref=self.balance_base_quat_ref,
            use_base_quat_ref=self.use_balance_base_quat_ref,
        )
        ...
        print(self.balance_foot_ref_anchor)
        jax.debug.print("reference: {}",reference[0,24:36])
    ...
    def reset(self,qpos,qvel):
        ...
        self._resample_balance_foot_ref(qpos, foot_op.flatten(), contact=None)
        self._update_swing_foot_goal(qpos)
        if self.use_balance_foot_ref_anchor:
            self.last_foot_ref = np.asarray(self.balance_foot_ref_anchor, dtype=np.float64)
        else:
    ...
    def set_balance_base_pose_reference(
        self,
        *,
        desired_height: float | None = None,
        desired_quat_wxyz=None,
        enabled: bool = True,
    ) -> None:
        """Set/clear a fixed desired base pose anchor used by balance reference generation."""
        self.use_balance_base_quat_ref = bool(enabled)
        if desired_quat_wxyz is not None:
            q = jnp.asarray(desired_quat_wxyz, dtype=jnp.float32).reshape(4)
            self.balance_base_quat_ref = q / (jnp.linalg.norm(q) + 1e-8)
        if desired_height is not None:
            self.robot_height = float(desired_height)

    def _resample_balance_foot_ref(self, qpos, foot_flat, contact=None):
        """Sample tripod foot XYZ refs in base frame (nominal + noise), projected to world."""
        cfg = self.config
        if not getattr(cfg, "use_tripod_nominal_foot_ref", False):
            self.use_balance_foot_ref_anchor = False
            return
        if not getattr(cfg, "use_balance_fixed_contact", False):
            self.use_balance_foot_ref_anchor = False
            return
        mask = jnp.asarray(cfg.balance_fixed_contact_mask, dtype=jnp.float32)
        if not bool(is_tripod_contact_mask(mask)):
            self.use_balance_foot_ref_anchor = False
            return
        p = jnp.asarray(qpos[:3], dtype=jnp.float32)
        quat = jnp.asarray(qpos[3:7], dtype=jnp.float32)
        foot_meas = jnp.asarray(foot_flat, dtype=jnp.float32)
        self._balance_foot_ref_key, subkey = jax.random.split(self._balance_foot_ref_key)
        sigma = getattr(cfg, "tripod_foot_ref_sigma", 0.03)
        self.use_balance_foot_ref_anchor = True
        self.balance_foot_ref_anchor = tripod_foot_reference_world(
            subkey,
            p,
            quat,
            cfg.p_legs0,
    ...
    def _update_swing_foot_goal(self, qpos) -> None:
        """
        Fixed viewer goal for the nominal swing leg (tripod / reduced support).

        Uses the random anchor when active; otherwise nominal ``p_legs0`` in the base frame.
        Independent of whether MPC tracks that ref each step.
        """
        cfg = self.config
        if not getattr(cfg, "use_balance_fixed_contact", False):
            self.swing_foot_goal_leg = -1
            return
        mask = np.asarray(cfg.balance_fixed_contact_mask, dtype=np.float64)
        swing_idx = [i for i in range(cfg.n_contact) if mask[i] < 0.5]
        if not swing_idx:
            self.swing_foot_goal_leg = -1
            return
        leg = int(swing_idx[0])
        if self.use_balance_foot_ref_anchor:
            anchor = np.asarray(self.balance_foot_ref_anchor, dtype=np.float64)
            self.swing_foot_goal_xyz = anchor[3 * leg : 3 * leg + 3].copy()
        else:
            p = np.asarray(qpos[:3], dtype=np.float64)
            quat = np.asarray(qpos[3:7], dtype=np.float64)
            foot_w = np.asarray(
                base_frame_feet_to_world(
                    jnp.asarray(p, dtype=jnp.float32),
                    jnp.asarray(quat, dtype=jnp.float32),
                    cfg.p_legs0,
                    cfg.n_contact,
                ),
                dtype=np.float64,
            )
            self.swing_foot_goal_xyz = foot_w[3 * leg : 3 * leg + 3].copy()
        self.swing_foot_goal_leg = leg

    def set_tripod_nominal_foot_ref(self, enabled: bool, qpos, foot_flat) -> bool:
        """Enable/disable random base-frame tripod foot refs; resample or clear anchor immediately."""
        cfg = self.config
        if not hasattr(cfg, "use_tripod_nominal_foot_ref"):
            return False
        cfg.use_tripod_nominal_foot_ref = bool(enabled)
        self._resample_balance_foot_ref(qpos, foot_flat)
        self._update_swing_foot_goal(qpos)
        if self.use_balance_foot_ref_anchor:
            self.last_foot_ref = np.asarray(self.balance_foot_ref_anchor, dtype=np.float64)
        else:
            self.last_foot_ref = np.asarray(foot_flat, dtype=np.float64)
        return bool(cfg.use_tripod_nominal_foot_ref and self.use_balance_foot_ref_anchor)

    def toggle_tripod_nominal_foot_ref(self, qpos, foot_flat) -> bool:
        """Flip ``use_tripod_nominal_foot_ref`` and refresh the MPC foot anchor."""
        cfg = self.config
        enabled = not bool(getattr(cfg, "use_tripod_nominal_foot_ref", False))
        return self.set_tripod_nominal_foot_ref(enabled, qpos, foot_flat)
