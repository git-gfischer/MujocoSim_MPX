...
import mpx.utils.math_utils.quad_math as quad_math
...
class MPCWrapper:
    ...
    def __init__(self, config, limited_memory=False):
        ...
        reference_generator = getattr(config, "reference_generator", reference_generator_balance)
        ...
        self._ref_gen = jax.jit(
            partial(
                ...
                fixed_contact_mask=config.balance_fixed_contact_mask,
            )
        )
    ...
    def run(self, data, x0, input, contact=None,
            base_quat_ref=None, use_base_quat_ref=None,
            foot_ref_anchor=None, use_foot_ref_anchor=None):

        contact = self.default_contact if contact is None else jnp.asarray(contact)
        if base_quat_ref    is None: base_quat_ref    = x0[3:7]
        if use_base_quat_ref is None: use_base_quat_ref = jnp.array(False)
        if foot_ref_anchor   is None: foot_ref_anchor   = x0[self.foot_slice]
        if use_foot_ref_anchor is None: use_foot_ref_anchor = jnp.array(False)
